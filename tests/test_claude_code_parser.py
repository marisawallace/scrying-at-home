...
class TestExtractSearchableText:
    ...
    def test_drops_machine_boilerplate_and_reconstructs_commands(self):
        lines = [
            # caveat blurb (isMeta) -> dropped
            {
                "type": "user",
                "isMeta": True,
            ...
            # slash command -> reconstructed to the typed form
            {
                "type": "user",
                "message": {"content": "<command-name>/usage</command-name>\n            <command-args></command-args>"},
                "timestamp": "t2",
            },
            # captured command output -> dropped
            {
                "type": "user",
                "message": {"content": "<local-command-stdout>some output</local-command-stdout>"},
                "timestamp": "t3",
            },
            # background-task notice -> dropped
            {
                "type": "user",
                "message": {"content": "<task-notification>\n<task-id>abc</task-id>\n</task-notification>"},
                "timestamp": "t4",
            },
            {"type": "user", "message": {"content": "Real prompt here"}, "timestamp": "t5"},
        ]
        texts = ccp.extract_searchable_text(lines)
        assert texts == ["/usage", "Real prompt here"]

    def test_reconstructs_message_first_command(self):
        # The exact harness shape that previously leaked raw tags: <command-message>
        # before <command-name>, and no <command-args>.
        lines = [
            {
                "type": "user",
                "message": {"content": "<command-message>code-review</command-message>\n<command-name>/code-review</command-name>"},
                "timestamp": "t1",
            },
        ]
        texts = ccp.extract_searchable_text(lines)
        assert texts == ["/code-review"]

    def test_excludes_meta_user_lines(self):
        lines = [
            {"type": "user", "isMeta": True, "message": {"content": "injected context"}, "timestamp": "t1"},
            {"type": "user", "message": {"content": "Actual prompt"}, "timestamp": "t2"},
        ]
        texts = ccp.extract_searchable_text(lines)
        assert texts == ["Actual prompt"]
...
class TestExtractConversationTurns:
    ...
    def test_drops_boilerplate_and_reconstructs_command_turns(self):
        lines = [
            {
                "type": "user",
                "isMeta": True,
        ...
        user_turns = [t for t in turns if t["role"] == "user"]
        assert [t["content"] for t in user_turns] == ["/code-review", "Real prompt here"]
...
class TestDeriveConversationName:
    ...
    def test_message_first_command_only_session_named_after_command(self):
        # Regression: <command-message> before <command-name>, no <command-args>.
        # This shape previously leaked the raw "<command-message>...</command-message>"
        # tag as the conversation name; it should reconstruct to "/code-review".
        lines = [
            {
                "type": "user",
                "isMeta": True,
    ...
    def test_skips_task_notification_lines(self):
        # Background-task-completion notices are injected as user turns but are
        # not typed by the human; they must not become the conversation name.
        lines = [
            {
                "type": "user",
                "message": {"content": "<task-notification>\n<task-id>abc</task-id>\n</task-notification>"},
            ...
            {"type": "user", "message": {"content": "Real prompt here"}, "timestamp": "t2"},
        ]
        name = ccp.derive_conversation_name(lines)
        assert name == "Real prompt here"
